...
export default function CameraController({ targetRef }: CameraControllerProps) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      angles.current.polar = Math.min(Math.max(angles.current.polar, 0.1), Math.PI / 2 - 0.1)
    }
  ...
  useFrame(() => {
    ...
    function getCameraPosition(target: THREE.Object3D, radius: number, azimuth: number, polar: number) {
      const x = target.position.x + radius * Math.sin(azimuth) * Math.cos(polar)
      const y = target.position.y + radius * Math.sin(polar) + 2  //+2 pra começar um pouco a cima do jogador
      const z = target.position.z + radius * Math.cos(azimuth) * Math.cos(polar)
      return { x, y, z }
    }

    const { x, y, z } = getCameraPosition(targetRef.current, radius, azimuth, polar)

    camera.position.set(x, y, z)
    camera.lookAt(targetRef.current.position.x, targetRef.current.position.y + 1, targetRef.current.position.z)
  })
